...
// configuration.
func Dsn() string {
	dsn := config.Get("queue_sqlite3_dsn")
	if dsn != "" {
		return dsn
	}
...
// New creates a jobqueue.Impl which uses SQLite3 as a data store.
...
func (q *jobQueue) Pop(limit uint) ([]jobqueue.Job, error) {
	...
	placeholders := make([]string, 0, limitInt)
	ids := make([]interface{}, 0, limitInt)

	// 1. Pre-SELECT jobs to grab.  We should not `SELECT ~ FOR UPDATE`
	// here because it blocks `Push()` due to a gap lock.
	if err := func() error {
		popQuery := fmt.Sprintf("%s %v", q.sql.grab, limitInt)

		rows, err := q.dbPop.Query(popQuery)
		if err != nil {
	...
	results := make([]jobqueue.Job, 0, limitInt)
	...
	if err := func() error {
		...
		rows, err := tx.Query(
			q.sql.grabbed+"("+strings.Join(placeholders, ",")+")",
			append([]interface{}{"claimed"}, ids...)...,
		)

		if err != nil {
			log.Debug().Msgf("Failed to select jobs: %v ", err)
			return err
		}
		defer rows.Close()
	...
	if err := func() error {
		log.Error().Msgf("query: %+v \n ids: %v", q.sql.launch+"("+strings.Join(placeholders, ",")+")", ids)
		_, err = tx.Exec(
...
func (q *jobQueue) connect() {
	...
	dbPop, err := sql.Open("sqlite3", q.dsn)
	if err != nil {
		log.Panic().Msgf("Cannot open DB: %s", err)
	}

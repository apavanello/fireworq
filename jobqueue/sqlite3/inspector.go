...
func (i *inspector) findAllDesc(status string, minTime int64, maxTime int64, limit uint, cursor string) (*jobqueue.InspectedJobs, error) {
	...
	var maxJobID int64 = math.MaxInt64
	if decoded, err := base64.StdEncoding.DecodeString(cursor); err == nil {
		if pair := strings.SplitN(string(decoded), ",", 2); len(pair) == 2 {
			t, err1 := strconv.Atoi(pair[0])
			j, err2 := strconv.Atoi(pair[1])
	...
	if err := func() error {
		...
		log.Info().Msgf(i.sql.inspectJobs + strconv.FormatInt(int64(limit)+1, 10))
		log.Info().Msgf("Inspecting jobs: %s, %d, %d, %d", status, minTime, maxTime, maxJobID)

		rows, err := i.db.Query(
			i.sql.inspectJobs+strconv.FormatInt(int64(limit)+1, 10),
			status,
			minTime,
			maxTime,
			maxJobID,
		)
		log.Info().Msgf("Inspecting jobs ROWs: %v", rows)

		if err != nil {
			log.Info().Msgf("Failed to inspect job: %s", err)
			return err
		}
		defer rows.Close()
		...
		log.Info().Msgf("Inspecting jobs IDs: %v", ids)
		return rows.Err()
	}(); err != nil {
		return nil, err
	}

	log.Info().Msgf("Inspecting jobs IDs LEN: %v", len(ids))
	if len(ids) <= 0 { // no job to report
		return &jobqueue.InspectedJobs{Jobs: results}, nil
	}
	...
	if err := func() error {
		...
		if err != nil {
			log.Error().Msgf("Failed to grab jobs: %s", err)
			return err
		}
		defer rows.Close()
		...
		log.Error().Msgf("Inspecting jobs err: %v", err)
		return nil, err
	}
...
func (i *inspector) scan(s scanner) (*jobqueue.InspectedJob, error) {
	...
	if err := s.Scan(&(j.ID), &(j.Category), &(j.URL), &stringPayload, &nextTry, &(j.Status), &createdAt, &retryCount, &(j.RetryDelay), &(j.FailCount), &(j.Timeout)); err != nil {
		return nil, err
	}

	if _, err := json.Marshal(stringPayload); err != nil {
		payload, _ := json.Marshal(stringPayload)
